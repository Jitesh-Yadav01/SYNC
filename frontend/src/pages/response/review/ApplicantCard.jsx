    ...
    >
      <div className="flex items-center gap-3.5">
        ...
        <div className="relative shrink-0">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-semibold border ${isSelected ? 'bg-white border-blue-500 text-blue-600' : 'bg-slate-100 border-slate-200 text-slate-700'}`}>
            {initials}
          </div>
          <div className="absolute -top-1.5 -left-1.5 bg-slate-900 text-white text-[10px] font-medium px-1.5 py-0.5 rounded shadow-sm">
            #{rank}
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex justify-between items-center mb-1">
            <h3 className={`text-sm font-semibold tracking-tight truncate pr-2 ${isSelected ? 'text-blue-700' : 'text-slate-900'}`}>{name}</h3>
            {averageScore > 0 && (
